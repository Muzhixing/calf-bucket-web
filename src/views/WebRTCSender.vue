<template>
  <div class="sender-root">
    ...
    <section class="panel">
      ...
      <div class="actions">
        <button class="primary" :disabled="isConnecting || isConnected" @click="start">
          {{ isConnecting ? '连接中...' : '开始推流' }}
        </button>
        <button class="ghost" :disabled="!isConnected && !isConnecting" @click="stop">停止</button>
      </div>
      <div class="status">
    ...
    <section class="stage">
      ...
      <div class="hint" v-if="!isConnected">未连接，点击“开始推流”</div>
    </section>
...
.sender-root {
  min-height: 100vh;
  background: radial-gradient(circle at 80% 10%, #1c2633, #0f1114 60%);
  color: #f1f1f1;
  padding: 24px;
  display: grid;
  gap: 18px;
